...
GSET is version **2.2.1**. It has **zero external dependencies** — one static Go binary.
...
The installer copies the binary to `~/.local/bin/gset`, adds `~/.local/bin` to your `PATH` (appends to `~/.bashrc` or `~/.zshrc`), and prints the installed version. Restart your shell (or `source ~/.bashrc`/`~/.zshrc`) if `gset` isn't found, then verify:
...
tar -xzf gset-2.2.1-linux-amd64.tar.gz
...
The build emits a `gset` binary in the repo root. To also copy it into `test/`, use `make build` (this produces `gset-2.2.1`).
...
| Debian/Ubuntu | `sudo dpkg -i gset_2.2.1_amd64.deb` (from releases) |
